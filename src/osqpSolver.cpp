...
{   //创建两倍大小的0值列向量
    ...
    if(!solver.data()->setHessianMatrix(hessian)) return;                  //设置目标函数二次型系数(Hessian矩阵)或p矩阵
    if(!solver.data()->setGradient(gradient)) return;                      //设置q or f矩阵。当没有时设置为全0向量
    if(!solver.data()->setLinearConstraintsMatrix(linearMatrix)) return;   //设置线性约束的A矩阵
    if(!solver.data()->setLowerBound(lowerBound)) return;                  //设置线性约束的下限
    if(!solver.data()->setUpperBound(upperBound)) return;                  //设置线性约束的上限
...
//0.5x'Hx+f'x=min
//lb<X<ub
//输入未平滑曲线，w_cost_smooth,w_cost_length,w_cost_ref 平滑代价 紧凑代价 几何相似度代价
//允许X,Y变化的上下界
//输出平滑后的参考线
/*
二次规划形式
        ...
        :size(config_.size), config(config_), plan_start(start_point)
{
    ...
    double d1 = config.host_d1;   //车辆中心到前部距离
    double d2 = config.host_d2;   //车辆中心到后部距离
    double w = config.host_w;     //车辆宽度
    ...
    solver.data()->setNumberOfVariables(NumberOfVariables);      //设置A矩阵列数 n
    solver.data()->setNumberOfConstraints(NumberOfConstraints);  //设置A矩阵行数 m
    if(!solver.data()->setHessianMatrix(hessian)) return;        //设置P矩阵
    if(!solver.data()->setGradient(gradient)) return;            //设置q或f矩阵，当没有时间设置为全0向量
    if(!solver.data()->setLinearConstraintsMatrix(linearMatrix)) return;   //设置线性约束的A矩阵
    if(!solver.data()->setLowerBound(lowerBound)) return;                  //设置下边界
    if(!solver.data()->setUpperBound(upperBound)) return;                  //设置上边界
